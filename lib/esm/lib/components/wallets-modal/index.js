var __makeTemplateObject = (this && this.__makeTemplateObject) || function (cooked, raw) {
    if (Object.defineProperty) { Object.defineProperty(cooked, "raw", { value: raw }); } else { cooked.raw = raw; }
    return cooked;
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
/* eslint-disable no-unused-vars */
import React, { useRef } from 'react';
import styled from 'styled-components';
import useAccountListener from '../../../hooks/useAccountListener';
import useOutsideClick from '../../../hooks/useOutsiteClick';
import { getConnectors, switchNetwork } from '../../../utils/storage';
var metamaskLogo = 'data:image/svg+xml;base64,';
var walletConnectLogo = 'data:image/svg+xml;base64,';
var Walletbutton_modal_item = styled.button(templateObject_1 || (templateObject_1 = __makeTemplateObject(["\nwidth: 100%;\npadding: 10px;\nborder: none;\nborder-radius: 5px;\nbackground-color: #fafafa;\nfont-size: 14px;\ncolor: #212121;\ncursor: pointer;\nmargin-bottom: 10px;\nbox-shadow: 0px 0px 5px rgba(0, 0, 0, 0.1);\ndisplay: flex;\nalign-items: center;\njustify-content: space-between;\n&:hover {\n  background-color: #f5f5f5;\n  box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.2);\n}\n"], ["\nwidth: 100%;\npadding: 10px;\nborder: none;\nborder-radius: 5px;\nbackground-color: #fafafa;\nfont-size: 14px;\ncolor: #212121;\ncursor: pointer;\nmargin-bottom: 10px;\nbox-shadow: 0px 0px 5px rgba(0, 0, 0, 0.1);\ndisplay: flex;\nalign-items: center;\njustify-content: space-between;\n&:hover {\n  background-color: #f5f5f5;\n  box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.2);\n}\n"])));
var Walletbutton_modal_title = styled.span(templateObject_2 || (templateObject_2 = __makeTemplateObject(["\nfont-size: 14px;\nfont-weight: bold;\ncolor: #212121;\nmargin-bottom: 25px;\nmargin-top: 15px;\n"], ["\nfont-size: 14px;\nfont-weight: bold;\ncolor: #212121;\nmargin-bottom: 25px;\nmargin-top: 15px;\n"])));
var Walletbutton_modal_container = styled.div(templateObject_3 || (templateObject_3 = __makeTemplateObject(["\ndisplay: flex;\nflex-direction: column;\njustify-content: center;\nalign-items: center;\nwidth: 250px;\nheight: fit-content;\nposition: absolute;\nleft: 50%;\ntop: 50%;\ntransform: translate(-50%, -50%);\npadding: 10px;\nmargin: 0px;\nborder: none;\nborder-radius: 5px;\nbackground-color: #fafafa;\nfont-size: 14px;\nfont-weight: bold;\ncolor: #000;\nbox-shadow: 0px 0px 5px rgba(0, 0, 0, 0.3);\n&:hover {\n  background-color: #f5f5f5;\n  box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.5);\n}\n@media (max-width: 768px) {\n  width: 80%;\n}\n"], ["\ndisplay: flex;\nflex-direction: column;\njustify-content: center;\nalign-items: center;\nwidth: 250px;\nheight: fit-content;\nposition: absolute;\nleft: 50%;\ntop: 50%;\ntransform: translate(-50%, -50%);\npadding: 10px;\nmargin: 0px;\nborder: none;\nborder-radius: 5px;\nbackground-color: #fafafa;\nfont-size: 14px;\nfont-weight: bold;\ncolor: #000;\nbox-shadow: 0px 0px 5px rgba(0, 0, 0, 0.3);\n&:hover {\n  background-color: #f5f5f5;\n  box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.5);\n}\n@media (max-width: 768px) {\n  width: 80%;\n}\n"])));
function WalletsModal(_a) {
    var _this = this;
    var _b;
    var showModal = _a.showModal, setShowModal = _a.setShowModal, activate = _a.activate, onSelect = _a.onSelect, setCurrentConnector = _a.setCurrentConnector, supportedChains = _a.supportedChains, chainId = _a.chainId;
    var activeInjected = function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!((window === null || window === void 0 ? void 0 : window.ethereum)
                        && !supportedChains.find(function (c) { return +c.chainId === +chainId; }))) return [3 /*break*/, 2];
                    return [4 /*yield*/, switchNetwork(supportedChains[0])];
                case 1:
                    _a.sent();
                    _a.label = 2;
                case 2:
                    activate(getConnectors().injected);
                    setCurrentConnector(0);
                    localStorage.setItem('walletIsConnected', 'true');
                    onSelect();
                    useAccountListener(window === null || window === void 0 ? void 0 : window.ethereum, function (accounts) {
                        console.log(accounts);
                        if (accounts.length > 0) {
                            localStorage.setItem('walletIsConnected', 'true');
                        }
                        else {
                            localStorage.setItem('walletIsConnected', 'false');
                            setCurrentConnector(-1);
                        }
                    });
                    return [2 /*return*/];
            }
        });
    }); };
    var activeWalletconnect = function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            activate(getConnectors().walletconnect);
            setCurrentConnector(1);
            onSelect();
            return [2 /*return*/];
        });
    }); };
    var modalContainer = useRef(null);
    useOutsideClick(modalContainer, function () {
        setShowModal(false);
    });
    return showModal ? (React.createElement("div", { style: {
            width: '100%',
            height: '100%',
            position: 'absolute',
            backgroundColor: 'rgba(0,0,0,0.5)',
            zIndex: '1',
            left: 0,
            top: 0,
        }, className: "walletbutton-modal-root" },
        React.createElement(Walletbutton_modal_container, { ref: modalContainer, className: "walletbutton-modal-container" },
            React.createElement(Walletbutton_modal_title, { className: "walletbutton-modal-title" }, "Select a wallet"),
            React.createElement(Walletbutton_modal_item, { className: "walletbutton-modal-item", onClick: activeInjected },
                "Metamask",
                React.createElement("img", { src: metamaskLogo, alt: "metamask", style: { width: '30px', height: '30px' } })),
            ((_b = getConnectors()) === null || _b === void 0 ? void 0 : _b.walletconnect) && (React.createElement(Walletbutton_modal_item, { className: "walletbutton-modal-item", onClick: activeWalletconnect },
                "Walletconnect",
                ' ',
                React.createElement("img", { src: walletConnectLogo, alt: "metamask", style: { width: '30px', height: '30px' } })))))) : (
    // eslint-disable-next-line react/jsx-no-useless-fragment
    React.createElement(React.Fragment, null));
}
export default WalletsModal;
var templateObject_1, templateObject_2, templateObject_3;
